...
const { rpc, apiVersion } = require('ddrive-daemon-client')
...
class HyperdriveDaemon extends EventEmitter {
  ...
  async _ready () {
    ...
    const seed = this.dwebstore.inner._deriveSecret(NAMESPACE, 'replication-keypair')
    const swarmId = this.dwebstore.inner._deriveSecret(NAMESPACE, 'swarm-id')
