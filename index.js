...
const { rpc, apiVersion } = require('./lib/dweb-peersockets/node_modules/ddrive-daemon-client')
...
const PeersocketManager = require('./lib/dweb-peersockets')
...
class HyperdriveDaemon extends EventEmitter {
  constructor (opts = {}) {
    ...
    this.dwebstore = new DWebstore(corestoreOpts.storage, corestoreOpts)
  ...
  async _ready () {
    ...
    await this.dwebstore.ready()

    const seed = this.dwebstore._deriveSecret(NAMESPACE, 'replication-keypair')
    const swarmId = this.dwebstore._deriveSecret(NAMESPACE, 'swarm-id')
    this._networkOpts.keyPair = HypercoreProtocol.keyPair(seed)
    this._networkOpts.id = swarmId

    this.networking = new SwarmNetworker(this.dwebstore, this._networkOpts)
    ...
    // Register the DWebswarm timeout heuristics on all cores generated by our dwebstore.
    ...
    this.drives = new DriveManager(this.dwebstore, this.networking, dbs.drives, {
      ...this.opts,
    ...
    this._versions = {
      ...
      client: require('ddrive-daemon-client/package.json').version,
      schema: require('ddrive-schemas/package.json').version,
      ddrive: require('ddrive/package.json').version
    }
    if (this.fuse && this.fuse.fuseConfigured) {
      this._versions.fuseNative = require('fuse-native/package.json').version
      this._versions.hyperdriveFuse = require('ddrive-fuse/package.json').version
    }
  }
    ...
    return {
      status: async call => {
        ...
        if (this._versions) {
          ...
          rsp.setHyperdriveversion(this._versions.ddrive)
  ...
  async stop (err) {
    ...
    try {
      ...
      log.info('waiting for dwebstore to close')
      if (this.dwebstore) {
