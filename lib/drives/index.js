...
const {
  ...
  setFileStats,
  toChunks
} = require('ddrive-daemon-client/lib/common')
...
const TRIE_UPDATER_SYMBOL = Symbol('ddrive-daemon-trie-updater')

class DriveManager extends Nanoresource {
  constructor (dwebstore, networking, db, opts = {}) {
    super()
  ...
  async get (key, opts = {}) {
    ...
    if (!drive) {
      ...
      const store = this.dwebstore.namespace(randomNamespace)
      drive = ddrive(store, key, {
        ...driveOpts
      })
  ...
  async _rpcPeerCounts (call) {
    ...
    for (let key of keys) {
      key = Buffer.from(key)
      if (this.dwebstore.isLoaded(key)) {
        const core = this.dwebstore.get(key)
